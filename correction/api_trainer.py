...
class APITrainer: 
    ...
    def get_prompt(self, batch, mode='get_opts'):
        ...
        if mode=='get_opts':
            prefix = 'In the following clinical note, what should the <BLANK>{blank_clause} be replaced with if "{potential_error_span}" is incorrect? Answer with 3 most promising options.'
            ...
            prefix = 'In the following clinical note, what should the <BLANK>{blank_clause} be replaced with for it to be medically informative and accurate? Choose one from the four options given below.'
