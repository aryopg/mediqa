...
def compute_accuracy(
    reference_flags, reference_sent_id, predicted_flags, candidate_sent_id
):
    ...
    for text_id in reference_sent_id:
        if (
            text_id in candidate_sent_id
            and candidate_sent_id[text_id] == reference_sent_id[text_id]
        ):
...
class NLGMetrics(object):
    ...
    def compute(self, references, predictions, counters):
        ...
        assert len(predictions) == len(
            references
        ), "Predictions and references do not have the same size."

        results["aggregate_subset_check"] = np.array(
        ...
        if "ROUGE" in self.metrics:
            ...
            results["aggregate_subset_check"] = results[
                "aggregate_subset_check"
            ] + np.array(rouge1f_scores)
            ...
            composite_score_rouge1 = (
                rouge1score + counters["system_provided_correct_na"]
            ) / counters["total_texts"]
            composite_score_rouge2 = (
                rouge2score + counters["system_provided_correct_na"]
            ) / counters["total_texts"]
            composite_score_rougeL = (
                rougeLscore + counters["system_provided_correct_na"]
            ) / counters["total_texts"]
        ...
        if "BERTSCORE" in self.metrics:
            ...
            composite_score_bert = (
                bertscores.sum() + counters["system_provided_correct_na"]
            ) / counters["total_texts"]
        ...
        if "BLEURT" in self.metrics:
            ...
            composite_score_bleurt = (
                bleurtscores.sum() + counters["system_provided_correct_na"]
            ) / counters["total_texts"]
            ...
            results["aggregate_subset_check"] = results[
                "aggregate_subset_check"
            ] + np.array(bleurtscores)

            aggregate_components += 1

        if aggregate_components > 0:
            aggregate_subset_scores = (
                results["aggregate_subset_check"] / aggregate_components
            )
            composite_score_agg = (
                aggregate_subset_scores.sum() + counters["system_provided_correct_na"]
            ) / counters["total_texts"]
...
def get_nlg_eval_data(
    reference_corrections, candidate_corrections, remove_nonprint=False
):
    references = []
    predictions = []
    ...
    for text_id in reference_corrections:
        ...
        if remove_nonprint:
            reference_correction = "".join(
                filter(lambda x: x in string.printable, str(reference_correction))
            )
...
if __name__ == "__main__":
    print("Testing the metrics module")
    ...
    candidate_corrections, candidate_flags, candidate_sent_id = (
        parse_run_submission_file(submission_file)
    )
